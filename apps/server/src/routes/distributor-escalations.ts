 ...
 *
 * POST  /api/distributor/escalations              — create escalation
 * GET   /api/distributor/escalations              — list (?status=pending|resolved, ?rep_id=)
 * PATCH /api/distributor/escalations/:id/resolve  — mark resolved with contact info
 * PATCH /api/distributor/escalations/:id/followup — rep acknowledges, removes from My Day
 */
import type { FastifyInstance } from "fastify";
import { getSql } from "../db/client.js";

export async function distributorEscalationsRoutes(app: FastifyInstance): Promise<void> {
  app.post<{ Body: Record<string, unknown> }>("/api/distributor/escalations", async (request, reply) => {
    const db = getSql();
    ...
    const [escalation] = await db`
      insert into distributor_escalations (salesperson_id, store_name, category, area, address, notes)
  ...
  app.get<{ Querystring: Record<string, string> }>("/api/distributor/escalations", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const rows = await db`
      ...
      from distributor_escalations e
      join distributor_salespeople sp on sp.id = e.salesperson_id
      where true
  ...
  app.patch<{
  ...
  }>("/api/distributor/escalations/:id/resolve", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [updated] = await db`
      update distributor_escalations set
        status                 = 'resolved',
  ...
  app.patch<{ Params: { id: string } }>("/api/distributor/escalations/:id/followup", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [updated] = await db`
      update distributor_escalations set followed_up_at = now()
