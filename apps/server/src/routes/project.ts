...
export async function projectRoutes(app: FastifyInstance): Promise<void> {
  ...
  // ── Salespeople ────────────────────────────────────────────────────────────
  app.get<{ Querystring: { active?: string } }>("/api/project/salespeople", async (request, reply) => {
    const db = getSql();
  ...
  // ── Lists ──────────────────────────────────────────────────────────────────
  app.get("/api/project/lists/categories", async (_req, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    const rows = await db<{ value: string }[]>`
      select value from project_visit_lists where type = 'category' and active = true order by value
    `;
    return { values: rows.map((r) => r.value) };
  });

  app.get("/api/project/lists/areas", async (_req, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    const rows = await db<{ value: string }[]>`
      select value from project_visit_lists where type = 'area' and active = true order by value
    `;
    return { values: rows.map((r) => r.value) };
  });

  // ── Customer suggest ───────────────────────────────────────────────────────
  app.get<{ Querystring: { q?: string } }>("/api/project/customers/suggest", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
  ...
  // ── List visits ────────────────────────────────────────────────────────────
  app.get<{ Querystring: Record<string, string> }>("/api/project/visits", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const rows = await db`
      ...
      from project_visits v
      join project_salespeople s on s.id = v.salesperson_id
      where true
  ...
  // ── Live stats ─────────────────────────────────────────────────────────────
  app.get("/api/project/visits/stats", async (_request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
  ...
  // ── Summary (count bar) ────────────────────────────────────────────────────
  app.get<{ Querystring: Record<string, string> }>("/api/project/visits/summary", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
  ...
  // ── XLSX export ────────────────────────────────────────────────────────────
  app.get<{ Querystring: Record<string, string> }>("/api/project/visits/export", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const filename = `project_kunjungan_${new Date().toISOString().slice(0, 10)}.xlsx`;
    return reply
  ...
  app.patch<{
  ...
  }>("/api/project/visits/:id", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [current] = await db<{ id: number; visited_at: Date }[]>`
      select id, visited_at from project_visits where id = ${id}
    `;
    if (!current) return reply.code(404).send({ error: "Visit not found." });

    const [saved] = await db`
      update project_visits set visited_at = ${new_ts} where id = ${id} returning *
    `;
    await db`
      insert into project_visit_audits (visit_id, field, old_value, new_value, changed_by)
  ...
  // ── Photo upload ───────────────────────────────────────────────────────────
  app.post<{ Params: { id: string } }>("/api/project/visits/:id/photos", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [visit] = await db<{ id: number }[]>`select id from project_visits where id = ${id}`;
    if (!visit) return reply.code(404).send({ error: "Visit not found." });

    const [countRow] = await db<{ n: string }[]>`
      select count(*)::text as n from project_visit_photos where visit_id = ${id}
    `;
    ...
    const [photo] = await db`
      insert into project_visit_photos (visit_id, file_data, mime_type, filename, file_size)
    ...
    return { ok: true, photo: { ...photo, url: `/api/project/photos/${photo.id}` } };
  });

  // ── Serve photo ────────────────────────────────────────────────────────────
  app.get<{ Params: { id: string } }>("/api/project/photos/:id", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [photo] = await db<{ file_data: Buffer; mime_type: string }[]>`
      select file_data, mime_type from project_visit_photos where id = ${id}
    `;
  ...
  app.get<{ Params: { id: string } }>("/api/project/visits/:id/photos", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const rows = await db`
      ...
      from project_visit_photos where visit_id = ${id} order by created_at
    `;
    return { photos: rows.map((r) => ({ ...r, url: `/api/project/photos/${r.id}` })) };
  });

  // ── Delete a photo ─────────────────────────────────────────────────────────
  app.delete<{ Params: { photoId: string } }>("/api/project/photos/:photoId", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [deleted] = await db`delete from project_visit_photos where id = ${photoId} returning id`;
  ...
  app.get<{
  ...
  }>("/api/project/insights/rep/:id", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [base] = await db<{
      ...
      from project_salespeople s
      left join project_visits v on v.salesperson_id = s.id
    ...
    const [catMix, areaMix, hourly] = await Promise.all([
      db<{ category: string; cnt: string }[]>`
        select category, count(*)::text as cnt from project_visits
        where salesperson_id = ${rep_id} and visited_at >= ${from} and visited_at <= ${to}
        group by category order by count(*) desc
      `,
      db<{ area: string; cnt: string }[]>`
        select area, count(*)::text as cnt from project_visits
        where salesperson_id = ${rep_id} and visited_at >= ${from} and visited_at <= ${to}
  ...
  // ── Insights: team ─────────────────────────────────────────────────────────
  app.get<{ Querystring: { from?: string; to?: string } }>("/api/project/insights/team", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
