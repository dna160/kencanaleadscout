 ...
 *
 * GET    /api/distributor/lists/categories      — all active categories
 * GET    /api/distributor/lists/areas           — all active areas
 * POST   /api/distributor/lists/:listType       — Handler+: add value
 * PATCH  /api/distributor/lists/:listType/:id   — Handler+: rename or deactivate
 */
...
export async function distributorListsRoutes(app: FastifyInstance): Promise<void> {
  app.get("/api/distributor/lists/categories", async (_request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    const rows = await db<{ id: number; value: string }[]>`
      select id, value from distributor_visit_lists
  ...
  app.get("/api/distributor/lists/areas", async (_request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    const rows = await db<{ id: number; value: string }[]>`
      select id, value from distributor_visit_lists
  ...
  // POST /api/distributor/lists/:listType — add a new value
  app.post<{
    Params: { listType: string };
    Body: { value?: string };
  }>("/api/distributor/lists/:listType", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    try {
      const [saved] = await db`
        insert into distributor_visit_lists (type, value)
        values (${type}, ${value})
  ...
  // PATCH /api/distributor/lists/:listType/:id — rename or deactivate
  app.patch<{
    Params: { listType: string; id: string };
    Body: { value?: string; active?: boolean };
  }>("/api/distributor/lists/:listType/:id", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [saved] = await db`
      update distributor_visit_lists set
