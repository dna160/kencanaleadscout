 ...
 *
 * GET    /api/distributor/accounts               — list with filters (stage, rep, area, type, q)
 * GET    /api/distributor/accounts/:id           — account card (+ action history, stage history, scheduled)
 * PATCH  /api/distributor/accounts/:id/stage     — change pipeline stage (writes stage_history)
 * POST   /api/distributor/accounts/:id/actions   — record touchpoint (kunjungan/telepon/received_order/note)
 * POST   /api/distributor/accounts/:id/schedule  — create scheduled follow-up
 * PATCH  /api/distributor/scheduled/:sid/done    — mark scheduled action complete
 * GET    /api/distributor/reps/:id/myday         — My Day: overdue / today / upcoming + at-risk
 */
...
export async function distributorAccountsRoutes(app: FastifyInstance): Promise<void> {
  // ── List accounts ─────────────────────────────────────────────────────────
  app.get<{ Querystring: Record<string, string> }>("/api/distributor/accounts", async (request, reply) => {
    const db = getSql();
    ...
    const rows = await db`
      select
        ...
        (
          select count(*) from distributor_actions a where a.account_id = c.id
        )::int as action_count
      from distributor_customers c
      left join distributor_salespeople sp on sp.id = c.owner_id
      where true
  ...
  // ── Account card ──────────────────────────────────────────────────────────
  app.get<{ Params: { id: string } }>("/api/distributor/accounts/:id", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [actions, history, scheduled] = await Promise.all([
      db`
        ...
        from distributor_actions a
        left join distributor_salespeople sp on sp.id = a.salesperson_id
      ...
      db`
        ...
        left join distributor_salespeople sp on sp.id = sh.changed_by
      ...
      db`
        select sa.id, sa.action_type, sa.scheduled_for, sa.notes, sa.completed_at,
               sp.full_name as salesperson_name, sp.code as salesperson_code
        from distributor_scheduled_actions sa
        left join distributor_salespeople sp on sp.id = sa.salesperson_id
  ...
  app.patch<{
  ...
  }>("/api/distributor/accounts/:id/stage", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [current] = await db<{ id: number; stage: string; account_type: string }[]>`
      select id, stage, account_type from distributor_customers where id = ${id}
    `;
    ...
    await Promise.all([
      db`update distributor_customers set stage = ${new_stage} where id = ${id}`,
      db`
    ...
    const [updated] = await db`select id, store_name, stage, account_type from distributor_customers where id = ${id}`;
  ...
  app.post<{
  ...
  }>("/api/distributor/accounts/:id/actions", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [account] = await db<{ id: number }[]>`select id from distributor_customers where id = ${id}`;
    if (!account) return reply.code(404).send({ error: "Account not found." });

    const [action] = await db`
      insert into distributor_actions (account_id, salesperson_id, action_type, invoice_number, notes)
  ...
  app.post<{
    Params: { id: string };
    Body: Record<string, unknown>;
  }>("/api/distributor/accounts/:id/schedule", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [scheduled] = await db`
      insert into distributor_scheduled_actions (account_id, salesperson_id, action_type, scheduled_for, notes)
  ...
  app.patch<{ Params: { sid: string } }>("/api/distributor/scheduled/:sid/done", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [updated] = await db`
      update distributor_scheduled_actions set completed_at = now()
  ...
  // ── My Day ────────────────────────────────────────────────────────────────
  app.get<{ Params: { id: string } }>("/api/distributor/reps/:id/myday", async (request, reply) => {
    const db = getSql();
    if (!db) return reply.code(503).send({ error: "Database not configured." });
    ...
    const [overdue, today, upcoming, atRisk, stats, portfolioRows, wonRows, resolvedEscalations, todayVisits, pipelineStale] = await Promise.all([
      ...
      db`
        ...
        from distributor_scheduled_actions sa
        join distributor_customers c on c.id = sa.account_id
      ...
      db`
        select
          (select count(*) from distributor_visits v
           where v.salesperson_id = ${rep_id}
             and v.visited_at >= ${todayStartUTC}
             and v.visited_at <  ${todayEndUTC}
          )::int as today_visits,
          (select count(*) from distributor_scheduled_actions sa
           where sa.salesperson_id = ${rep_id}
             and sa.completed_at is null
             and sa.scheduled_for < ${todayStartUTC}
          )::int as overdue_count,
          (select count(*) from distributor_customers c
           where c.owner_id = ${rep_id}
             and c.account_type = 'repeating'
             and c.stage <> 'repeat_order'
             and c.last_contact_at is not null
      ...
      db`
        ...
        left join distributor_customers c on c.id = v.customer_id
        where v.salesperson_id = ${rep_id}
          and v.visited_at >= ${todayStartUTC}
          and v.visited_at <  ${todayEndUTC}
 ...
 */
export async function startDistributorCadenceEngine(): Promise<void> {
  const db = getSql();
  if (!db) return;
  ...
  async function tick(): Promise<void> {
    ...
    try {
      ...
      const toAktif = await sql<{ id: number; old_stage: string }[]>`
        with target as (
          select id, stage as old_stage from distributor_customers
          where account_type = 'repeating'
        ...
        set stage = 'aktif'
        from target
        where distributor_customers.id = target.id
        returning distributor_customers.id, target.old_stage
      `;

      if (toFollowup.length > 0 || toAtRisk.length > 0 || toAktif.length > 0) {
        for (const r of toFollowup) {
          await sql`insert into distributor_stage_history (account_id, old_stage, new_stage, changed_by) values (${r.id}, 'aktif', 'perlu_followup', null)`;
        }
        for (const r of toAtRisk) {
          await sql`insert into distributor_stage_history (account_id, old_stage, new_stage, changed_by) values (${r.id}, 'perlu_followup', 'at_risk', null)`;
        }
        for (const r of toAktif) {
          await sql`insert into distributor_stage_history (account_id, old_stage, new_stage, changed_by) values (${r.id}, ${r.old_stage}, 'aktif', null)`;
        }
        console.info(
